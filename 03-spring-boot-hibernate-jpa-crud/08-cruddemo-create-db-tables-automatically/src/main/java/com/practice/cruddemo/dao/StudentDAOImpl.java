...
public class StudentDAOImpl implements StudentDAO {// creating a class 'StudentDAOImpl' which implements 'StudentDAO' interface to use the method 'save'.
    ...
    @Override
    @Transactional              // Because we are deleting objects from table.
    public int deleteAll() {
        return entityManager.createQuery("DELETE FROM Student").executeUpdate();    // it will return total number of rows that got deleted.
    }
